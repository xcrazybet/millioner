// ============================================
// betting-engine.js - CANCELLATION + CASHOUT
// ============================================
...
async function placeAccumulatorBet(selections, amount) {
    const user = firebase.auth().currentUser;
    if (!user) return { success: false, error: 'Login required' };
    if (selections.length < 2) return { success: false, error: 'Min 2 selections' };
    ...
    try {
        ...
        await db.collection('bets').add({
            userId: user.uid,
            selections, amount, totalOdds, potentialWin,
...
async function cancelBetWithFee(betId) {
    ...
    try {
        ...
        const feePercent = window.BET_CANCELLATION_RULES?.getCancelFee(match.startTime) || 100;
...
async function cashoutBet(betId) {
    ...
    try {
        ...
        const matchDoc = await db.collection('sports_matches').doc(bet.fixtureId.toString()).get();
        const match = matchDoc.data();
        if (match.status !== 'live') return { success: false, error: 'Cashout only available during live match' };
        
        const minute = Math.floor((new Date() - match.startTime.toDate()) / 60000);
        const feePercent = window.BET_CANCELLATION_RULES?.getCashoutFee(minute) || 25;
        
        // Cashout value = potential win × (current win probability) - fee
...
console.log('✅ Betting Engine - Cancellation & Cashout Active');
