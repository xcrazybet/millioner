// ============================================
// betting-engine.js - v9.1 SUPABASE
// Accumulator popup + All bet types
// ============================================
...
window.addAccSelection = function(fixtureId, homeName, awayName, betType, betTypeName, odds) {
    ...
    window.ACCUMULATOR_SLIP.push({
        fixtureId: fixtureId, matchName: `${homeName} vs ${awayName}`,
        betType: betType, betTypeName: betTypeName, odds: odds
    });
    saveSlip();
...
async function placeAccumulatorBet(selections, amount) {
    ...
    try {
        ...
        for (const s of selections) {
            ...
            if (!m) return { success: false, error: 'Match not found: ' + s.matchName };
...
console.log('✅ Betting Engine v9.1 - Secure');
